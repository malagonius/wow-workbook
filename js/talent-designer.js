...
function sectionNodes(section) {
  ...
  // Compatibility with the current row-based schema. Empty strings become real
  // editor nodes so the designer can immediately work with Lichborn.
  const nodes = [];
...
function nodePosition(node, section) {
  ...
  const x = ((node.column + .5) / cols) * 100;
  const y = ((node.row + .5) / rows) * 100;
  return { x, y };
}
...
function render() {
  ...
  treeEl.innerHTML = `<div class="designer-canvas" style="aspect-ratio:${cols}/${Math.max(rows, 1)}">\n    <svg class="designer-svg" aria-hidden="true"></svg>\n    ${nodes.map(node => {
      ...
      return `<div class="designer-node-wrap" style="left:${p.x}%;top:${p.y}%;transform:translate(-50%,-50%)">
        <button class="designer-node ${classes.slice(1).join(' ')}" data-node-id="${node.id}" title="${node.name || node.id}"><span class="designer-node-label">${label || ''}</span></button>
      </div>`;
    }).join('')}\n  </div>`;
...
function updatePanel() {
  ...
  $('node-row').value = node.row ?? 0;
  $('node-column').value = node.column ?? 0;
...
function addNode() {
  const section = currentSection();
  const nodes = sectionNodes(section);
  const id = `${section.id}-${nodes.length}`;
...
function saveNode(event) {
  ...
  if (!newId) return;
  if (newId !== node.id && sectionNodes(section).some(item => item !== node && item.id === newId)) {
  ...
  Object.assign(node, {
    ...
    row: Number($('node-row').value) || 0,
    column: Number($('node-column').value) || 0
  });
...
function deleteNode() {
  ...
  if (Array.isArray(list)) {
    connections[currentKey()][section.id] = list.filter(pair => {
      const from = `${section.id}-${pair[0].split('-').slice(1).join('-')}`;
      const to = `${section.id}-${pair[1].split('-').slice(1).join('-')}`;
...
function addConnection(from, to) {
  ...
  connections[key] ||= {};
  connections[key][currentSection().id] ||= [];
  const section = currentSection();
  const fromRaw = from.startsWith(`${section.id}-`) ? from : `${section.id}-${from}`;
  const toRaw = to.startsWith(`${section.id}-`) ? to : `${section.id}-${to}`;
  const pair = [fromRaw, toRaw];
  if (!connections[key][section.id].some(existing => existing[0] === pair[0] && existing[1] === pair[1])) {
    connections[key][section.id].push(pair);
  }
}

function showJson() {
  $('json-output').value = JSON.stringify(currentTree(), null, 2);
...
function applyJson() {
  try {
    ...
    config.trees[currentKey()] = replacement;
    updateSections();
    $('json-dialog').close();
  } catch (error) {
...
function validate() {
  ...
  if (result.valid && !result.warnings.length) panel.innerHTML = '<span class="validation-ok">✓ Tree section is valid</span>';
  else panel.innerHTML = `${result.errors.map(e => `<div class="validation-error">✕ ${e}</div>`).join('')}${result.warnings.map(w => `<div class="validation-warning">⚠ ${w}</div>`).join('')}`;
}

async function init() {
  const [treeResponse, connectionResponse] = await Promise.all([fetch(TREES_URL), fetch(CONNECTIONS_URL)]);
  config = await treeResponse.json();
  ...
  $('connect-mode').addEventListener('click', () => { connectMode = !connectMode; connectSource = null; $('connection-panel').hidden = !connectMode; render(); });
...
init().catch(error => {
  treeEl.innerHTML = `<p class="validation-error">Unable to load designer data: ${error.message}</p>`;
});
